...
int main()
{
	...
	while (option != EXIT) {
		...
		/* menu item 2 - 14 requires image is loaded first */
			...
			ColorFilter(image, int target_r, int target_g, int target_b, int threshold, int replace_r, int replace_g, int replace_b);
			...
			AddBorder(image, color, border_width);
					...
					case 17:
						...
						scanf("%d", &contast);
						BrightnessAndContast(image, brightness, contrast);
...
void PrintMenu() {
	...
	printf("17: Adjust the Brightness and Contast of an image\n");
